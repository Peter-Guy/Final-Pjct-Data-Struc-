...
struct fiveCardStudPlayer
{
    ...
    int kicker = 0; //Stores the kicker card in the event of a tie
...
bool fiveCardStud::isStraightOrFlush(int player)
{
    int counter[13] = {0};
    ...
    if(isRoyal)
    {
        ...
        if(isFlush(player))
        {
            ...
            {
                //Royal flush
                players[player].handScore = 180;
            }
            else
            {
                //Ace low straight flush
                players[player].handScore = 160 + highestCard;
            }
            return true;
        }
        //Not a flush
        ...
        {
            //Straight
            players[player].handScore = 80 + highestCard;
            return true;
        }
    }
    ...
    else
    {
        ...
        {
            //Flush
            players[player].handScore = 100 + highestCard;
            return true;
        }
        //Not a flush
        else
...
void fiveCardStud::multipleKind(int player)
{
    ...
    for(int i = 0; i < 13; i++) 
    {
        ...
        if(counter[i] == 2) 
        {
            ...
            {
                //Two pair
                players[player].handScore = i + 40;
                players[player].kicker = numPair;
                return;
            }
            ...
            {
                //Full house
                players[player].handScore = 60 + threeKind;
                players[player].kicker = i;
                return;
            }
        ...
        else if(counter[i] == 3)
        {
            ...
            {
                //Full house
                players[player].handScore = i + 120;
                players[player].kicker = numPair;
                return;
            }
            else 
        ...
        {
            //Four of a kind
            players[player].handScore = 140 + i;
            return;
        }
        //Stores highest card
        else if (counter[i] == 1) 
    ...
    {
        //One pair
        players[player].handScore = 20 + numPair;

        //Searches for the highest card in reverse to find the kicker
        for(int i = 12; i >= 0; i--)
        {
            if(counter[i] == 1)
            {
    ...
    {
        //High card
        players[player].handScore = highCard + 0;

        //Searches for the highest card in reverse to find the kicker
        for(int i = highCard; i >= 0; i--)
        {
            if(counter[i] == 1)
            {
                players[player].kicker = i;
...
//If two players have a matching hand, use kicker cards to decided who wins or
//If they split
int fiveCardStud::kickerCardCheck()
{
    int winningPlayer = -1; //Initialize to -1 to indicate no winner yet

    //Go through all players and check their hand against the next
    for(int i = 0; i < playerCount; i++) 
    {
        for(int j = i + 1; j < playerCount; j++) 
        {
            //Players have a matching hand
            if (players[i].handScore == players[j].handScore) 
            {
                // The first player won due to a higher kicker card
                if(players[i].kicker > players[j].kicker) 
                {
                    winningPlayer = i;
                }
                //They split the pot, this works for straights and the likes
                else if(players[i].kicker == players[j].kicker) 
                {
                    winningPlayer = -1; // Indicate a tie/split
                }
                //The second player won due to a higher kicker card
                else
                {
                    winningPlayer = j;
                }
    ...
    //No players had a matching hand
    return 0;
}
...
void playFiveCardStud()
{
    ...
    do
    {
        ...
        winningPlayer = mainGame.kickerCardCheck();

        //Procede as normal if no matching hands
        if(winningPlayer == 0)
        {
