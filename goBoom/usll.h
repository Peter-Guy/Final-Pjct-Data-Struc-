...
class usll {
    public:
        ...
        bool find(const int&, card&) const;
        bool makeEmpty();
        int getLength() const;
        bool isEmpty() const;
        bool isFull();
...
bool usll::find(const int &num, card &c) const {
    if(isEmpty() || num > qty || num < 1) return false;
    
    //Increment through all values in the list
    for(const node *scan = start; scan != nullptr; scan = scan->next) {
        if (scan->cardNum == num) {
            c = scan->_c;
            return true;
        }
